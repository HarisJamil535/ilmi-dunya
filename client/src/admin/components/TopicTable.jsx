...
import { Search, FileText, Filter, X, ChevronLeft, ChevronRight, Video, ExternalLink } from "lucide-react";
                ...
                <table className="w-full text-left text-sm border-collapse">
                    <thead>
                        <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-600 uppercase text-xs tracking-wider">
                            <th className="px-6 py-4 font-bold w-28">Topic #</th>
                            ...
                            paginatedTopics.map((topic, index) => (
                                ...
                                >
                                    <td className="px-6 py-4 align-middle">
                                        <div className="inline-flex items-center justify-center w-11 h-11 rounded-xl bg-primary-soft text-primary font-bold font-mono text-sm border border-primary-soft shadow-2xs">
                                            {topic.topicNumber || 1}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 align-middle">
                                        {topic.videoUrl ? (
                                            ...
                                            >
                                                <Video className="w-6 h-3.5 shrink-0" /> Watch Video <ExternalLink className="w-6 h-3 shrink-0" />
                                            </a>
                                        ) : (
                                    ...
                                    <td className="px-6 py-4 align-middle text-right whitespace-nowrap">
                                        <div className="flex items-center justify-end gap-2">
                                            <EditButton onClick={() => onEdit(topic)} />
                                            <DeleteButton onClick={() => onDelete(topic)} />
                                        </div>
                                    </td>
                                </tr>
                            ))
